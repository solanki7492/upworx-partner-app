...
import { AvailabilityState, DayKey } from '@/lib/types/service';
...
export default function AvailabilityScreen() {
  ...
  const loadAddressData = async () => {
    try {
      const res = await getPartnerAddress();
      if (res.status && res.data) {
        setAddressData({
          latitude: res.data.latitude,
          longitude: res.data.longitude,
          range_area: res.data.range_area,
          address_line_1: res.data.address_line_1,
        });
      }
    } catch (error) {
      console.log('Failed to load address data:', error);
    }
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {DAYS.map((day) => (
          <View key={day} style={styles.card}>
            ...
            <View style={styles.timeRow}>
              ...
              >
                <Text style={styles.timeText}>{availability[day].start}</Text>
              ...
              >
                <Text style={styles.timeText}>{availability[day].end}</Text>
                <Ionicons name="time-outline" size={18} />
              </TouchableOpacity>
            </View>
          </View>
        ...
        {addressData && (
          <View style={styles.mapContainer}>
            <Text style={styles.mapTitle}>Service Area</Text>
            <View style={styles.mapWrapper}>
              ...
              >
                <Marker
                  ...
                  description={addressData.address_line_1}
                />
                <Circle
                  center={{
                    latitude: addressData.latitude,
                  ...
                  radius={addressData.range_area * 1000} // Convert km to meters
                  fillColor="rgba(97, 122, 249, 0.2)"
                  strokeColor="rgba(97, 122, 249, 0.5)"
                  strokeWidth={2}
                />
              </MapView>
            </View>
            <View style={styles.mapInfo}>
              ...
              <Text style={styles.mapInfoText}>
                Service area: {addressData.range_area} km radius
              </Text>
            </View>
          </View>
        )}
